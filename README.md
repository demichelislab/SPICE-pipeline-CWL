...
variants namely: SNVs, indels and copy number variations. CLONET makes possible
to estimate ploidy (average number of copies of the genome in the
sequencing data) and purity (fraction of tumor reads in the sequencing data) of
the tumor tissue. Purity and ploidy are known to affect copy number values and
allelic fractions of SNVs. Using such purity and ploidy estimates CLONET is
able to generate corrected copy numbers and SNVs. Moreover CLONET outputs
allele specific copy numbers (number of copies of the parental alleles).

To allow a reliable analysis, the SPICE CWL pipeline includes several QC tools.
The output generated by QC tools help the user to check the quality of the data
and correctness of the configuration.

The CWL implementation make easier to run the pipeline on heterogeneous
computational resources (e.g. user machines, clusters, cloud) and to easily
scale upon need. All the included tools are packed in containers to improve
ease of use and enable reproducibility.
